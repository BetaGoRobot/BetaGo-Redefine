...
type PlannerMessageContext struct {
	Direct       bool
	MentionedBot bool
}
...
你会额外看到消息元信息 MessageContext：
- direct_addressing: 当前消息是否可以视为在直接找机器人接话
- mentioned_bot: 当前消息是否明确 @ 到机器人
这些字段是可靠信号，不是普通上下文噪音。
当 direct_addressing=true 或 mentioned_bot=true 时，说明用户更可能希望机器人立刻接住当前请求；但工具类型仍必须由消息语义本身决定，不要把 direct/mentioned 机械映射成某个固定工具。
...
func PlanTools(ctx context.Context, chatID, openID string, modelID, currentInput string, recentLines []string, scope llmusage.Scope) ([]intentmeta.ToolHint, error) {
	return PlanToolsWithContext(ctx, chatID, openID, modelID, currentInput, recentLines, PlannerMessageContext{}, scope)
}

func PlanToolsWithContext(
	ctx context.Context,
	chatID, openID string,
	modelID, currentInput string,
	recentLines []string,
	msgCtx PlannerMessageContext,
	scope llmusage.Scope,
) ([]intentmeta.ToolHint, error) {
	...
	userPrompt := buildToolPlannerUserPrompt(currentInput, recentLines, msgCtx)
...
func buildToolPlannerUserPrompt(currentInput string, recentLines []string, msgCtx PlannerMessageContext) string {
	var b strings.Builder
	b.WriteString("消息元信息:\n")
	b.WriteString("direct_addressing: ")
	if msgCtx.Direct {
		b.WriteString("true\n")
	} else {
		b.WriteString("false\n")
	}
	b.WriteString("mentioned_bot: ")
