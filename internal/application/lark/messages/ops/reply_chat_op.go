...
func (r *ReplyChatOperator) PreRun(ctx context.Context, event *larkim.P2MessageReceiveV1, meta *xhandler.BaseMetaData) (err error) {
	...
	if *event.Event.Message.ChatType != "p2p" && !msg.IsMentioned(event.Event.Message.Mentions) {
		return errors.Wrap(consts.ErrStageSkip, r.Name()+" Not Mentioned")
	}

	if command.LarkRootCommand.IsCommand(ctx, msg.PreGetTextMsg(ctx, event)) {
		return errors.Wrap(consts.ErrStageSkip, r.Name()+" Not Mentioned")
	}
	return
}
...
func (r *ReplyChatOperator) Run(ctx context.Context, event *larkim.P2MessageReceiveV1, meta *xhandler.BaseMetaData) (err error) {
	...
	reactionID, err := lark_dal.AddReaction(ctx, "OnIt", *event.Event.Message.MessageId)
	if err != nil {
