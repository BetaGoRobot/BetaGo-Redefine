...
9. interaction_mode 固定为 standard（当前仅保留标准聊天链路）：
   - standard: 单轮问答、寒暄、解释、轻聊天、简单追问、单点事实查询。

10. reasoning_effort 用于给后续标准对话提供思考深度建议：
   - minimal: 几乎不需要推理，简单接话或直接执行单步任务
   - low: 需要少量分析，任务较明确
   - medium: 需要多步分析或权衡
   - high: 明显要求深入分析、复杂规划、强约束推演

11. tool_hints 提示后续生成阶段优先调用的工具集合（可空，可多选）：
   ...
   仅在确实需要时返回；不需要任何工具时返回空数组 []。need_reply=false 时必须返回 []。
...
func analyzeMessage(ctx context.Context, message string, recentLines []string, modelID string, scope llmusage.Scope) (analysis *IntentAnalysis, err error) {
	...
	logs.L().Ctx(ctx).Info("intent analysis completed",
		...
		zap.Any("tool_hints", analysis.ToolHints),
	)
