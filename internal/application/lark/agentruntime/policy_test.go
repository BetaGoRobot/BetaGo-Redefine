...
func TestDefaultGroupPolicyMentionSupersedesActiveRun(t *testing.T) {
	...
	if decision.SupersedeRunID != "run_active" {
		t.Fatalf("expected supersede active run, got %#v", decision)
	}
}
...
func TestDefaultGroupPolicyAllowsFollowUpWithinWindow(t *testing.T) {
	...
	active := &ActiveRunSnapshot{
		...
		Status:       RunStatusWaitingCallback,
...
func TestDefaultGroupPolicyCommandBridgeSupersedesActiveRun(t *testing.T) {
	policy := NewDefaultGroupPolicy(DefaultGroupPolicyConfig{
		FollowUpWindow: 2 * time.Minute,
	})
	now := time.Unix(1710000000, 0).UTC()
	...
	if decision.SupersedeRunID != "run_active" {
		t.Fatalf("expected /bb to supersede active run, got %#v", decision)
	}
}
