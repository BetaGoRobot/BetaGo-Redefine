...
func TestBuildAgenticChatPromptContextUsesReplyScopedContext(t *testing.T) {
	...
	scopedMessages := history.OpensearchMsgLogList{
		{
			...
			OpenID:     "ou_user",
			UserName:   "Alice",
			MsgList:    []string{"请把 agentic runtime 的最后一步展开"},
		},
		{
			CreateTime: "2026-03-20 10:01:00",
			OpenID:     "ou_bot",
			UserName:   "Agent",
			MsgList:    []string{"最后一步是把首轮 model turn 也纳入 durable runtime。"},
		},
	}
	...
	agenticChatRecallDocs = func(ctx context.Context, chatID, query string, topK int) ([]schema.Document, error) {
		...
		return []schema.Document{
			{
				PageContent: "reply scoped recalled context",
...
func TestDefaultAgenticChatReplyScopeLoaderIncludesParentChainAndRuntimeState(t *testing.T) {
	...
	agenticChatRunLookupByResponseMessage = func(ctx context.Context, messageID string) (*agenticReplyRunState, error) {
		...
		return &agenticReplyRunState{
			...
			Goal:          "推进 agentic runtime 理想态",
			InputText:     "请继续推进 agentic 改造",
			ResultSummary: "已经发起审批，等待继续执行",
		}, nil
	}
	agenticChatRunStepsLoader = func(ctx context.Context, runID string) ([]*AgentStep, error) {
		if runID != "run_1" {
		...
		return []*AgentStep{
			{
				...
				InputJSON: []byte(`{
					"thought_text":"先收敛上下文，再把首轮 turn durable 化",
					"reply_text":"下一步是把首轮 turn 也沉淀为 plan step。",
					"pending_capability":{
			...
			},
			{
				...
				OutputJSON: []byte(`{
					"thought_text":"先收敛上下文，再把首轮 turn durable 化",
					"reply_text":"最后一步是让首轮 model turn 也进入 durable runtime。",
					"response_message_id":"om_parent"
				}`),
			},
		}, nil
	}
	...
	if !strings.Contains(lines[0], "请继续推进 agentic 改造") {
		t.Fatalf("first line = %q, want contain root message", lines[0])
	}
	if !strings.Contains(lines[1], "最后一步是让首轮 model turn 也进入 durable runtime。") {
		t.Fatalf("second line = %q, want contain runtime reply text", lines[1])
	}

	contextText := strings.Join(scoped.ContextLines, "\n")
	for _, want := range []string{
		"关联运行状态: waiting_approval",
		"关联计划: 推进 agentic runtime 理想态",
		"最近一轮计划:",
		"下一步是把首轮 turn 也沉淀为 plan step。",
		"待执行能力: send_message {\"text\":\"继续推进\"}",
		"最近一轮思考: 先收敛上下文，再把首轮 turn durable 化",
	} {
		if !strings.Contains(contextText, want) {
			t.Fatalf("context = %q, want contain %q", contextText, want)
		}
	}
	if !strings.Contains(scoped.RecallQuery, "请继续推进 agentic 改造") ||
		!strings.Contains(scoped.RecallQuery, "最后一步是让首轮 model turn 也进入 durable runtime。") ||
		!strings.Contains(scoped.RecallQuery, "下一步是把首轮 turn 也沉淀为 plan step。") {
		t.Fatalf("recall query = %q, want contain parent chain", scoped.RecallQuery)
	}
}
