...
type capabilityReplyOutput struct {
	ThoughtText string `json:"thought_text,omitempty"`
	ReplyText   string `json:"reply_text,omitempty"`
}

const (
	// DefaultInitialChatToolTurns is an exported chat flow constant.
	DefaultInitialChatToolTurns = 8
	// DefaultAgenticInitialChatToolTurns is an exported chat flow constant.
	DefaultAgenticInitialChatToolTurns = 8
	// DefaultAgenticResearchToolTurns is an exported chat flow constant.
	DefaultAgenticResearchToolTurns = 12
	stepKindPlan                    = "plan"
	stepKindReply                   = "reply"
	stepStatusFailed                = "failed"
	stepStatusSkipped               = "skipped"
	replyLifecycleStateSuperseded   = "superseded"
)

var (
	agenticChatHistoryLoader     = defaultAgenticChatHistoryLoader
	agenticChatReplyScopeLoader  = defaultAgenticChatReplyScopeLoader
	agenticChatReplyThreadLoader = defaultAgenticChatReplyThreadLoader
	...
	agenticChatReplyScopeThreadMessageLimit = 8
)

// BuildAgenticChatExecutionPlan implements chat flow behavior.
func BuildAgenticChatExecutionPlan(ctx context.Context, req InitialChatGenerationRequest) (InitialChatExecutionPlan, error) {
	...
	return InitialChatExecutionPlan{
		...
		ReasoningEffort: intent.NormalizeReasoningEffort(req.ReasoningEffort, intent.InteractionModeAgentic),
		ChatID:          chatID,
		OpenID:          openID,
		Prompt:          agenticChatSystemPrompt(),
		UserInput:       buildAgenticChatUserPrompt(promptCtx),
		MaxToolTurns:    ResolveResearchToolTurnLimit(promptCtx.UserRequest),
...
func buildAgenticChatPromptContext(
	...
	return messageList, agenticChatPromptContext{
		...
		ResearchMode: looksLikeResearchRequest(userRequest),
	}, nil
}

// ResolveResearchToolTurnLimit implements chat flow behavior.
func ResolveResearchToolTurnLimit(texts ...string) int {
	for _, text := range texts {
		if looksLikeResearchRequest(text) {
			return DefaultAgenticResearchToolTurns
		}
	}
	return DefaultAgenticInitialChatToolTurns
}

func looksLikeResearchRequest(text string) bool {
	text = strings.ToLower(strings.TrimSpace(text))
	if text == "" {
		return false
	}
	for _, keyword := range []string{"deep research", "research", "investigate", "citation", "citations", "sources", "source link", "with sources", "with links", "compare sources", "read the docs", "深度调研", "深挖", "调研", "研究", "出处", "引用", "来源", "链接", "对比", "综述", "文献", "论文", "资料"} {
...
func defaultAgenticChatReplyScopeLoader(ctx context.Context, event *larkim.P2MessageReceiveV1) (agenticReplyScopeContext, bool, error) {
	if scoped, ok, err := buildAgenticThreadReplyScopeContext(ctx, event); err != nil {
		return agenticReplyScopeContext{}, false, err
	} else if ok {
		return scoped, true, nil
	}
	return buildAgenticLegacyReplyScopeContext(ctx, event)
}

func buildAgenticThreadReplyScopeContext(ctx context.Context, event *larkim.P2MessageReceiveV1) (agenticReplyScopeContext, bool, error) {
	if strings.TrimSpace(replyThreadID(event)) == "" || agenticChatReplyThreadLoader == nil {
		return agenticReplyScopeContext{}, false, nil
	}
	messages, err := agenticChatReplyThreadLoader(ctx, event)
	if err != nil {
		return agenticReplyScopeContext{}, false, err
	}
	messages = normalizeReplyScopedThreadMessages(messages, replyCurrentMessageID(event))
	if len(messages) == 0 {
		return agenticReplyScopeContext{}, false, nil
	}
	if limit := agenticChatReplyScopeThreadMessageLimit; limit > 0 && len(messages) > limit {
		return agenticReplyScopeContext{}, false, nil
	}
	return buildAgenticReplyScopeContextFromMessages(ctx, messages)
}

func buildAgenticLegacyReplyScopeContext(ctx context.Context, event *larkim.P2MessageReceiveV1) (agenticReplyScopeContext, bool, error) {
...
func defaultAgenticChatReplyThreadLoader(ctx context.Context, event *larkim.P2MessageReceiveV1) ([]*larkim.Message, error) {
	if strings.TrimSpace(replyThreadID(event)) == "" {
		return nil, nil
	}
	return larkmsg.GetAllParentMsg(ctx, event)
}

func buildAgenticReplyScopeContextFromMessages(ctx context.Context, messages []*larkim.Message) (agenticReplyScopeContext, bool, error) {
	messageList := make(history.OpensearchMsgLogList, 0, len(messages))
	recallParts := make([]string, 0, len(messages)*2)
	contextLines := make([]string, 0, len(messages)*2)
	for _, message := range messages {
		runtimeCtx, err := resolveAgenticReplyRuntimeContextForMessage(ctx, message)
		if err != nil {
			return agenticReplyScopeContext{}, false, err
		}
		if messageLog := buildAgenticReplyScopedMessageLog(message, runtimeCtx); messageLog != nil {
			messageList = append(messageList, messageLog)
			recallParts = append(recallParts, strings.Join(messageLog.MsgList, " "))
		}
		if runtimeCtx != nil {
			contextLines = append(contextLines, runtimeCtx.ContextLines()...)
			recallParts = append(recallParts, runtimeCtx.RecallQueryParts()...)
		}
	}
	if len(messageList) == 0 && len(contextLines) == 0 {
		return agenticReplyScopeContext{}, false, nil
...
func resolveAgenticReplyRuntimeContextForMessage(ctx context.Context, message *larkim.Message) (*agenticReplyRuntimeContext, error) {
	messageID := strings.TrimSpace(agenticFetchedMessageID(message))
	if messageID == "" {
		return nil, nil
	}
	return resolveAgenticReplyRunContext(ctx, messageID)
}

func normalizeReplyScopedThreadMessages(messages []*larkim.Message, currentMessageID string) []*larkim.Message {
	filtered := make([]*larkim.Message, 0, len(messages))
	...
	sort.SliceStable(filtered, func(i, j int) bool {
		left := replyScopedMessageCreateTimeMillis(filtered[i])
		right := replyScopedMessageCreateTimeMillis(filtered[j])
		if left == right {
			return strings.TrimSpace(agenticFetchedMessageID(filtered[i])) < strings.TrimSpace(agenticFetchedMessageID(filtered[j]))
		}
		return left < right
	})
	return filtered
...
func replyScopedMessageCreateTimeMillis(message *larkim.Message) int64 {
	if message == nil || message.CreateTime == nil {
		return 0
	}
	value, err := strconv.ParseInt(strings.TrimSpace(*message.CreateTime), 10, 64)
	if err != nil {
		return 0
	}
	return value
...
func defaultAgenticChatTopicSummaryLookup(ctx context.Context, chunkIndex, msgID string) (string, error) {
	...
	resp, err := opensearch.SearchData(ctx, chunkIndex, osquery.Search().Sort("timestamp_v2", osquery.OrderDesc).Query(osquery.Bool().Must(osquery.Term("msg_ids", msgID))).Size(1))
	if err != nil {
		return "", err
	}
...
func defaultAgenticChatRunStepsLoader(ctx context.Context, runID string) ([]*agenticReplyStep, error) {
	ins := infraDB.QueryWithoutCache()
	...
	steps := make([]*agenticReplyStep, 0, len(entities))
	for _, entity := range entities {
		steps = append(steps, &agenticReplyStep{
...
// ContextLines implements chat flow behavior.
func (c *agenticReplyRuntimeContext) ContextLines() []string {
	if c == nil || c.RunState == nil {
		return nil
	}
	lines := make([]string, 0, 4)
	if status := strings.TrimSpace(c.RunState.Status); status != "" {
...
// RecallQueryParts implements chat flow behavior.
...
func latestAgenticReplyTexts(steps []*agenticReplyStep) (string, string) {
...
func latestAgenticPlanStep(steps []*agenticReplyStep) *replyPlanStepInput {
	for i := len(steps) - 1; i >= 0; i-- {
		step := steps[i]
...
func decodeAgenticReplyTexts(output []byte) (string, string) {
	...
	capability := capabilityReplyOutput{}
...
func replyLifecycleState(step *agenticReplyStep) string {
	...
	return strings.TrimSpace(reply.LifecycleState)
}

func replyParentMessageID(event *larkim.P2MessageReceiveV1) string {
	return message.ParentID(event)
}
func replyThreadID(event *larkim.P2MessageReceiveV1) string { return message.ThreadID(event) }
func replyCurrentMessageID(event *larkim.P2MessageReceiveV1) string {
	return message.MessageID(event)
}
...
func agenticFetchedMessageID(message *larkim.Message) string {
...
- 如果还没有读过页面正文，或者来源数量、来源多样性、关键问题覆盖度还不够，就视为证据不足；证据不足时继续调用工具，而不是直接给结论。
...
func buildAgenticChatUserPrompt(ctx agenticChatPromptContext) string {
	...
	if ctx.ResearchMode {
		builder.WriteString("\n研究要求:\n")
		builder.WriteString(agenticChatLinesBlock([]string{
			"这是 research/调研型请求，默认不要在第一次搜索后直接收尾。",
			"如果需要先找来源，优先用 web_search；打开正文用 research_read_url；抽证据用 research_extract_evidence；整理引用用 research_source_ledger。",
			"只有在关键问题已覆盖、来源足够且证据不再明显缺失时才输出最终结论。",
		}))
	}
