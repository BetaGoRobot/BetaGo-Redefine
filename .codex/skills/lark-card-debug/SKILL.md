...
```bash
.codex/skills/lark-card-debug/scripts/send_card.sh --list-scenes
```

直接按 canonical scene key 构建某个场景：
...
跑一轮 smoke suite，并把结果写成 JSON：

```bash
.codex/skills/lark-card-debug/scripts/send_card.sh \
  --suite smoke \
  --dry-run \
  --report-json /tmp/card-regression.json
```
...
- `--spec`: 使用仓库内置调试卡片。
- `--scene`: 使用 canonical scene key，例如 `schedule.query`、`permission.manage`。
- `--case`: 指定 scene case，例如 `smoke-default`、`live-default`。
- `--suite`: 批量执行回归套件，例如 `smoke`、`live-smoke`、`send-smoke`。
...
- 如果要验证当前已纳入协议的真实场景，优先 `--scene` / `--suite`，不要继续新增 ad-hoc `--spec`。
